// ── match-worker/cupDraw.ts ──────────────────────────────────────────────────
// Verbatim copy of src/features/match/logic/cupDraw.ts so the Deno worker can
// use the bracket-draw engine without resolving the project's path aliases.
// Pure TS with zero external imports — kept identical to the src copy so
// either runtime produces the same bracket for the same input.
//
