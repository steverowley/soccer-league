...
// Voice corpus — Phase 0 extraction of the in-engine `buildCommentary` template
// pools.  The wrapper below (`buildCommentary`) is the only consumer in this
// file; the corpus module is a pure read-only TypeScript module so it can also
// be exercised by unit tests without booting the engine.
import { pickCommentary } from './features/agents/logic/commentaryCorpus';
...
// ── buildCommentary ───────────────────────────────────────────────────────────
// Thin compatibility wrapper around `pickCommentary` in
// `src/features/agents/logic/commentaryCorpus.ts`.
//
// WHY this function is now a wrapper:
//   The template pools + picker + weirdness gate moved to the agent
//   system's voice-corpus module (Phase 0 of the agent plan).  Keeping the
//   engine's public API identical means the rest of `gameEngine.js`, every
//   `simulateHelpers` caller, the worker, and all UI consumers continue to
//   call `buildCommentary(type, actors, outcome, flavour, ctx)` with no
//   change.  Math.random() is consumed in the same order so existing
//   seeded tests (e.g. `gameEngine.smoke.test.ts`) remain byte-identical.
//
// The contract — for new callers — is:
//   • type     — 'shot' | 'freekick' | 'penalty' | 'header' | 'tackle'
//   • actors   — { attacker?, defender? } names
//   • outcome  — 'goal' | 'saved' | 'miss' | 'post' for shot-likes;
//                'won' | 'contested' | 'lost' for tackle
//   • flavour  — string array from `resolveContest()`; tags map to bool
//                flags inside the corpus (exhausted, clutch, anxious,
//                ecstatic, confident, creative, low_confidence)
//   • ctx      — { min, scoreDiff, playerGoals, isArchitectFeatured }
//                all defaulted; isArchitectFeatured raises the weirdness
//                rate from 3% to 8%.
export function buildCommentary(type, actors, outcome, flavour = [], ctx = {}) {
  return pickCommentary(type, actors, outcome, flavour, ctx);
}
