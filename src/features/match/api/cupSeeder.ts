 ...
 *
 * WHY: the seeder used to read a hardcoded list of Season-1 league UUIDs, so it
 * only ever worked for Season 1. Season 2+ competitions get fresh random UUIDs
 * (see seasonRollover.ts), so we resolve them dynamically by
...
export async function resolveSeasonLeagueCompIds(
  ...
  return rows
    .filter((r) => r.league_id !== null && (LEAGUE_ID_ORDER as readonly string[]).includes(r.league_id))
    .sort(
      (a, b) =>
        (LEAGUE_ID_ORDER as readonly string[]).indexOf(a.league_id as string) -
        (LEAGUE_ID_ORDER as readonly string[]).indexOf(b.league_id as string),
    )
    .map((r) => r.id);
}
 ...
 */
export async function resolveSeasonCupCompIds(
  db: IslSupabaseClient,
  seasonId: string,
