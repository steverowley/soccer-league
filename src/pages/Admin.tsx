 ...
 * (bypassing the ~30 s GitHub Actions push-to-main cron path).
 *
 * The board pulls its own Supabase client via `useSupabase()` and
 * subscribes to `bd_issues` via Realtime, so after a successful resync
 * the cards re-render in place without a refetch wire-up here.
 *
 * The button is admin-only by virtue of being inside the admin tab; the
 * Edge Function ALSO checks `profiles.is_admin` on the JWT so a
 * compromised UI guard alone can't fire the sync.
 */
function RoadmapPanel() {
  const db = useSupabase();

  // ── Local state: in-flight + last-result chip ─────────────────────────
  // `busy` disables the button and shows the spinner-ish "…" label while
  // the function is running.  `result` is a small chip that fades in
  // beside the button after a resync; either a success summary or an
  // error message.  Cleared on the next click so back-to-back clicks
  // don't leave stale chrome.
  const [busy, setBusy]     = useState(false);
  const [result, setResult] = useState<{ kind: 'success' | 'error'; message: string } | null>(null);

  /**
   * Trigger an on-demand resync.  Calls the `bd-sync-now` Edge Function
   * via the Supabase client — the client automatically attaches the
   * caller's JWT in the `Authorization` header so the function can
   * gate on `profiles.is_admin`.
   *
   * The function returns `{ upserted, deleted, synced_at }` on success.
   * Any non-2xx response carries an `error` code we surface verbatim in
   * the inline chip so admins can self-diagnose (e.g. `not_admin`,
   * `github_fetch_failed`, `tombstone_cap_exceeded`).
   */
