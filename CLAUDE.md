...
2. Open the PR via `mcp__github__create_pull_request` if one doesn't exist
3. Subscribe to PR activity (`mcp__github__subscribe_pr_activity`) so review comments and CI failures wake the session
4. Close any GitHub issues finished this session
5. Do NOT end a session that opened a PR without subscribing
